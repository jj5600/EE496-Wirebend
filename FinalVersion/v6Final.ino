...
//----------------------------------
  ...
  digitalWrite(43,LOW);   // turn the LED on (HIGH is the voltage level)
  digitalWrite(45,LOW); 
  digitalWrite(47,LOW); 
  digitalWrite(49,LOW); 
  digitalWrite(51,LOW); 

  attachInterrupt(digitalPinToInterrupt(20),ISRT, LOW);
     ...
     if(Serial1.available() > 0) // if available data is greater than zero
       {
        ...
        if(data.startsWith("RESET")) //RESET BUTTON
        {
          start_reset=true;
        }
        else { start_reset=false;}
...
{
  exit(0);
  //attachInterrupt(digitalPinToInterrupt(20), ISR, LOW);
}
//////////////////////////////////////////////////////////////////
void progress()
{
...
bool ohmmeter ()
{
  ...
  int i=0;
      ...
      if(s[i].flag==false)
      {
        ...
        if(s[i].sensorValue==1023) //s[i].R>p20 || s[i].R<m20 || s[i].sensorValue==1023
          { 
            if(s[i].z==0)
              {
                s[i].flag=true;
